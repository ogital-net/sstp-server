...
const OPTSTRING: &str =
    "l:(listen)s:(secret)p:(pool)d:(dns)u:(user)r:(rate-limit)v+(verbose)qh(help)";
...
Options:
  ...
  -r, --rate-limit <spec> Mikrotik-Rate-Limit VSA (vendor 14988, attr 8)
                          to attach to every Access-Accept. Free-form
                          per the RouterOS grammar; common shapes:
                            512k/256k               (rx/tx flat)
                            '1M 2M 1M 30 8 512k'    (full grammar)
                          See sstp-server's shape::mikrotik docs.
...
impl Handler for PapHandler {
    async fn handle(&self, request: Request<'_>) -> HandlerResult {
        ...
        // Mikrotik-Rate-Limit VSA (vendor 14988, attr 8). Attached
        // verbatim to every Access-Accept when --rate-limit is set,
        // so sstp-server's RADIUS bridge can install per-session
        // HTB egress + ingress policing on the pppN/tun netdev.
        if let Some(spec) = self.rate_limit.as_deref() {
            use radius_tokio::typed::{VsaAttr, WText};
            let handle = VsaAttr::<WText>::new(14988, 8);
            if let Err(e) = reply.add_vsa(handle, spec) {
                eprintln!("[{src}] failed to add Mikrotik-Rate-Limit: {e:?}");
                return HandlerResult::Drop;
            }
        }
...
async fn run() -> Result<(), String> {
    ...
    for opt in getopt.by_ref() {
        match opt.val() {
            ...
            'r' => {
                rate_limit = Some(opt.arg().expect("getopt guarantees arg").to_string());
            }
            'v' => verbose += 1,
            'q' => quiet = true,
            '?' => {
    ...
    if let Some(spec) = &rate_limit {
        eprintln!("dev-radius: Mikrotik-Rate-Limit = {spec:?}");
    }
