...
4. TLS 1.3 `KeyUpdate` and `NewSessionTicket` records are detected
   ...
   `SSTP_EVT_TLS_REKEY_NEEDED`; userspace cooperatively reinstalls
   fresh crypto info via `setsockopt`.
...
[kernel-abi/sstp.h](../kernel-abi/sstp.h) (ABI v0.2, frozen for
the v0.x series).
...
frames (Echo-Request, Call-Disconnect, etc.) and TLS 1.3
KeyUpdate.

On Linux hosts with `sstp.ko` loaded and a kTLS-eligible cipher
negotiated (TLS 1.2 / 1.3 + AES-GCM — see
...
- **kmod** ([kmod/](../kmod/)) — v0.2 RC, ABI frozen, full
  per-frame data path implemented in C. Builds against any Linux
  6.12+ tree with `CONFIG_PPP=m`. **Functional and tested to
  pass traffic.**
...
1. **TLS 1.3 KeyUpdate / NewSessionTicket continuation.**
   ...
   classifier). Userspace currently logs a TODO and tears the
   session down
   ([src/session.rs](../src/session.rs) `handle_kmod_event` rekey
   arm). For 1.0: pause the kmod data path, pull the new traffic
   secrets out of `aws-lc-sys`, re-install fresh `crypto_info`
   via `setsockopt(SOL_TLS, TLS_TX/TLS_RX, ...)`, and resume.
   Without this, long-lived sessions die on the first
   `KeyUpdate` (TLS 1.3 endpoints SHOULD rekey periodically per
   RFC 8446 §4.6.3).

2. **kTLS support for ChaCha20-Poly1305.**
   [`KtlsEligibility`](../src/crypto/tls.rs) only allow-lists
   AES-GCM suites; [src/crypto/ktls.rs](../src/crypto/ktls.rs)
   only implements `install_aes_gcm_128` /
   `install_aes_gcm_256`. Sessions negotiating a ChaCha suite
   (common on ARM clients without AES-NI) silently fall back to
   the TUN path. Adding `TLS_CIPHER_CHACHA20_POLY1305` install
   support keeps the fast path on for those clients.
...
4. **Outbound-control parity.**
   [`SSTP_IOC_RECV_CONTROL`](../kernel-abi/sstp.h) drains
   inbound `C=1` frames from the kmod to userspace. There is no
   matching kmod ioctl for *sending* control frames — outbound
   SSTP control today goes via the userspace TLS write path,
   which only works because the kmod is happy to share the
   socket. For 1.0 either document this asymmetry as
   intentional or add `SSTP_IOC_SEND_CONTROL` so userspace
   never touches the socket post-attach (eliminates a class of
   send-vs-rekey races).

5. **`--data-path kernel` strict-mode CI.**
...
6. **`PppBackend` Drop order: explicit, not implicit.**
   The current order is "whatever struct-field declaration
   order gives us" ([src/kppp/session.rs](../src/kppp/session.rs)
   `PppBackend` definition). Add an explicit `impl Drop for
   PppBackend` (or document the ordering invariant in a
   `// SAFETY:`-style comment on the field declarations) so a
   future field-reorder doesn't silently swap detach-then-remove
   with remove-then-detach.

7. **TUN egress routing.**
   The TUN backend's host→peer direction needs a route on the
   server pointing at `tunN` for the client's assigned
   `Framed-IP-Address`. Today that route is implicit (the
   `RTM_NEWADDR` P2P pair installs a /32 host route to the
   peer); confirmed working for the e2e ICMP echo, but no
   guidance for the multi-session / pool-of-clients case.
   For 1.0: document the deployment expectation (either
   per-session /32s as today, or a single /N route managed
   externally) and either add `Framed-Route` (`RTM_NEWROUTE`)
   support or note the omission.

8. **NP-mode filtering parity.**
   The kernel backend inherits `ppp_generic`'s NP-mode filter
   (the kernel won't dispatch a frame for a protocol the unit
   hasn't enabled). The TUN backend bypasses `ppp_generic`
   entirely — a misbehaving peer can push e.g. an IPX frame
   into the SSTP stream and we'd happily PPP-decode and drop
   it, but there's no policy enforcement layer. Not a security
   issue (raw IP into `tunN` is just dropped by the host
   stack), but a behavioural divergence between the two
   backends worth either closing or documenting.

9. **Comment / scope drift.**
   [src/crypto/ktls.rs](../src/crypto/ktls.rs) header comment
   still says "AES-128 only" even though `install_aes_gcm_256`
   has landed alongside it. Cosmetic, but the kind of drift
   that bites the next reader.

## See also

- [CLAUDE.md](../CLAUDE.md) — top-level architecture, the
  authoritative source for design rationale.
