//! Shared `NETLINK_*` primitives for the `shape` module.
//!
//! Sister of [`crate::kppp::netlink`], scoped to the two consumers
//! that live inside `shape`: [`super::netlink`] (NETLINK_ROUTE for
//! `tc`) and [`super::mss`] (NETLINK_NETFILTER for nf_tables MSS
//! clamping). Hosts:
//!
//! - The `NLM_F_*` / `NLMSG_ERROR` / `NLMSG_HDRLEN` constants `libc`
//!   does not export.
//! - [`bytes_of`] — the POD-as-bytes helper, replacing four ad-hoc
//!   `slice::from_raw_parts` sites that previously lived inline in
//!   the two callers.
//! - [`MessageBuf`] — the wire-format builder. One outer `nlmsghdr`
//!   per buffer (always at offset 0) plus a stack of nested
//!   `nlattr` brackets. nf_tables and rtnetlink share an identical
//!   on-wire shape; the small differences in attribute helper names
//!   between the two old types collapsed to one set.
//! - [`NetlinkSocket`] — a minimal `socket(2) + bind(2) + send + recv`
//!   wrapper around an `OwnedFd`. Synchronous; never used from the
//!   data path.
//! - [`drain_acks`] — walks every `nlmsghdr` in a reply buffer,
//!   consuming each ack from a `HashSet<u32>` of expected sequence
//!   numbers. Single-message and multi-message (nf_tables batch)
//!   callers use the same routine.
//!
//! Why not reuse [`crate::kppp::netlink`] directly: that type's
//! `MessageBuf` / `exchange` surface is private to the `kppp`
//! module and grew up around `RTM_NEWADDR` / `RTM_NEWLINK` shapes.
//! Promoting it would expand the `kppp` API surface for two
//! adjacent consumers; keeping a small focused mirror here is
...
/// View a POD `T: Copy` as its raw byte representation. Replaces the
/// four ad-hoc `slice::from_raw_parts(ptr.cast(), size_of::<T>())`
/// sites that previously lived inline in the two old netlink stacks.
#[must_use]
...
/// followed by a flat or nested attribute tree.
///
...
impl MessageBuf {
    ...
    /// `nfgenmsg`, …) that follow the outer `nlmsghdr`.
    ...
    pub fn nest_end(&mut self) {
        let off = self
            .nest_stack
            .pop()
            .expect("nest_end without nest_begin");
...
pub fn drain_acks(buf: &[u8], remaining: &mut HashSet<u32>) -> Result<(), DrainError> {
    ...
    while offset + NLMSG_HDRLEN <= buf.len() {
        ...
        let hdr = unsafe {
            std::ptr::read_unaligned(buf[offset..].as_ptr().cast::<libc::nlmsghdr>())
        };
