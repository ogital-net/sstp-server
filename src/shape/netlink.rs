...
//! live one level up in [`super::wire`]; this module is a thin
//! tc-flavoured façade over them.
//!
//! ## Scope
//!
...
use super::wire::{self, DrainError, NetlinkSocket};
...
pub use super::wire::MessageBuf;
pub use super::wire::{NLM_F_ACK, NLM_F_CREATE, NLM_F_REPLACE, NLM_F_REQUEST};
    ...
    pub fn exchange(&self, op: &'static str, buf: &MessageBuf) -> Result<(), ShapeError> {
        ...
        match wire::drain_acks(received, &mut remaining) {
            ...
            Err(DrainError::Truncated) => {
                Err(ShapeError::Netlink(io::Error::other("truncated netlink ack")))
            }
