...
fn encode_ingress_police_filter(seq: u32, ifindex: u32, spec: &RateSpec) -> netlink::MessageBuf {
    ...
    // Rate table: kernel `tcf_police_init` always calls
    // `qdisc_get_rtab` when `parm->rate.rate != 0`, and that
    // helper short-circuits to `NULL` (→ `-ENOMEM`) unless the
    // `tc_ratespec` carries `linklayer == TC_LINKLAYER_UNAWARE`,
    // a non-zero `cell_log`, and a 1024-byte `TCA_POLICE_RATE`
    // attribute. We compute it the same way iproute2 does.
    let cell_log = compute_cell_log(mtu);
    let rtab = compute_rate_table(rate_bps_u64.max(1), cell_log);
    ...
    let mut parm = TcPolice {
        ...
        rate: TcRatespec {
            cell_log,
            // TC_LINKLAYER_UNAWARE (=0); required for the kernel
            // to accept the rate table emitted in TCA_POLICE_RATE.
            linklayer: 0,
            overhead: 0,
            // iproute2 sets cell_align = -1 to signal "table is
            // self-describing"; the kernel doesn't otherwise
            // consult this field.
            cell_align: -1,
            mpu: 0,
            rate: u32::try_from(rate_bps_u64.min(u64::from(u32::MAX))).expect("u32 saturate"),
    ...
    // 256-entry u32 rate table (1024 bytes). Required by
    // `tcf_police_init` → `qdisc_get_rtab`; without it the
    // kernel returns `-ENOMEM` even though no allocation
    // actually fails.
    buf.push_attr_bytes(police::TCA_POLICE_RATE, rate_table_bytes(&rtab).as_slice());
...
/// `cell_log` for a 256-entry rate table covering packets up to
/// `mtu` bytes. iproute2 picks the smallest `n` such that
/// `mtu >> n <= 255` (so every packet size up to MTU has a slot
/// in the 256-entry table). Required by `qdisc_get_rtab` to be
/// non-zero.
...
/// Compute the 256-entry HTB/police rate table. Each entry is
/// the transmit time in PSCHED ticks for a packet of size
/// `(i + 1) << cell_log` at `rate_bytes_per_sec`.
///
/// On modern kernels (`clock_res = 1e9` in `/proc/net/psched`)
/// one tick equals one nanosecond, matching what
/// [`burst_bytes_to_psched_ticks`] assumes.
fn compute_rate_table(rate_bytes_per_sec: u64, cell_log: u8) -> [u32; 256] {
    let mut tab = [0u32; 256];
    for (i, slot) in tab.iter_mut().enumerate() {
        let sz = u64::from((i as u32 + 1) << cell_log);
        // ticks (ns) = size / rate * NSEC_PER_SEC
        let ticks = sz.saturating_mul(1_000_000_000) / rate_bytes_per_sec;
        *slot = u32::try_from(ticks.min(u64::from(u32::MAX))).expect("clamped to u32");
    }
    tab
}

/// Render a 256-entry rate table as 1024 bytes of native-byte-order
/// `u32`s, ready to push as a netlink attribute payload.
fn rate_table_bytes(tab: &[u32; 256]) -> [u8; 1024] {
...
mod tests {
    use super::*;
    use crate::shape::tc::{self, police};
    ...
    #[test]
    fn compute_cell_log_picks_smallest_n_for_256_slots() {
        // For every n returned, (mtu >> n) must be <= 255 and
        // (mtu >> (n-1)) must be > 255 (or n must equal the
        // documented floor of 1).
        for &mtu in &[64u32, 256, 512, 1280, 1500, 2047, 2048, 4096, 9000] {
            let n = compute_cell_log(mtu);
            assert!(n >= 1, "cell_log must be non-zero (got {n} for mtu {mtu})");
            assert!(
                (mtu >> n) <= 255,
                "(mtu={mtu}) >> (n={n}) = {} must be <= 255",
                mtu >> n
            );
            if n > 1 {
                assert!(
                    (mtu >> (n - 1)) > 255,
                    "n={n} not minimal for mtu={mtu}: (mtu>>(n-1))={}",
                    mtu >> (n - 1)
                );
            }
        }
        // mtu==0 falls back to 2047 (matches iproute2 default).
        assert_eq!(compute_cell_log(0), compute_cell_log(2047));
        // Spot-check the canonical 1500-byte Ethernet MTU since
        // that's the value the encoder hardcodes today.
        assert_eq!(compute_cell_log(1500), 3);
    }

    #[test]
    fn compute_rate_table_is_monotonic_and_matches_formula() {
        // 1 Mbit/s = 125_000 bytes/s.
        let rate = 125_000u64;
        let cell_log = 3u8;
        let tab = compute_rate_table(rate, cell_log);

        // Strictly non-decreasing — bigger packets take more ticks.
        for w in tab.windows(2) {
            assert!(
                w[1] >= w[0],
                "rate table must be non-decreasing: {} -> {}",
                w[0],
                w[1]
            );
        }

        // Modern PSCHED tick == 1ns, so rtab[i] ≈ size_ns/rate.
        // Spot-check entry 0 and 255.
        let sz0 = 1u64 << cell_log; // 8 bytes
        let sz255 = 256u64 << cell_log; // 2048 bytes
        let expected0 = sz0 * 1_000_000_000 / rate;
        let expected255 = sz255 * 1_000_000_000 / rate;
        assert_eq!(u64::from(tab[0]), expected0);
        assert_eq!(u64::from(tab[255]), expected255);
    }

    #[test]
    fn compute_rate_table_saturates_on_micro_rates() {
        // Pathological 1 byte/s rate would overflow u32 ticks for
        // big slots; the helper must saturate, never wrap or panic.
        let tab = compute_rate_table(1, compute_cell_log(1500));
        assert_eq!(tab[255], u32::MAX);
        assert!(tab[0] > 0);
    }

    #[test]
    fn rate_table_bytes_is_native_u32_layout() {
        let mut tab = [0u32; 256];
        tab[0] = 0xDEAD_BEEF;
        tab[255] = 0xCAFE_BABE;
        let bytes = rate_table_bytes(&tab);
        assert_eq!(bytes.len(), 1024); // 256 * 4
        assert_eq!(&bytes[0..4], &0xDEAD_BEEFu32.to_ne_bytes());
        assert_eq!(&bytes[1020..1024], &0xCAFE_BABEu32.to_ne_bytes());
    }

    #[test]
    fn encode_ingress_police_emits_required_rate_table() {
        // Regression guard for the kernel `tcf_police_init` →
        // `qdisc_get_rtab` requirement: without a 1024-byte
        // `TCA_POLICE_RATE` attribute the kernel returns ENOMEM.
        let spec = RateSpec::flat(2_000_000); // 2 Mbit
        let buf = encode_ingress_police_filter(/* seq */ 21, /* ifindex */ 5, &spec);
        let bytes = buf.bytes();

        // Hunt every 4-byte alignment for the attribute header
        // {nla_len = 1028, nla_type = TCA_POLICE_RATE}. Linear
        // scan is fine: the rate table is unique by length.
        let mut found = false;
        let mut i = 0;
        while i + 4 <= bytes.len() {
            let nla_len = u16::from_ne_bytes(bytes[i..i + 2].try_into().unwrap());
            let nla_type = u16::from_ne_bytes(bytes[i + 2..i + 4].try_into().unwrap());
            if nla_len == 1028
                && nla_type == police::TCA_POLICE_RATE
                && i + nla_len as usize <= bytes.len()
            {
                found = true;
                break;
            }
        ...
        assert!(found, "TCA_POLICE_RATE (1024-byte rate table) missing");
    }

    #[test]
    fn encode_ingress_police_ratespec_uses_unaware_linklayer() {
        // Kernel `qdisc_get_rtab` short-circuits to NULL unless
        // tc_ratespec.linklayer == TC_LINKLAYER_UNAWARE (=0) and
        // cell_log != 0. Locate the TCA_POLICE_TBF attribute by
        // scanning every 4-byte alignment for its header
        // {nla_len = 4 + sizeof(tc_police), nla_type = 1}; the
        // body is the `tc_police` struct.
        let spec = RateSpec::flat(10_000_000);
        let buf = encode_ingress_police_filter(/* seq */ 22, /* ifindex */ 7, &spec);
        let bytes = buf.bytes();

        let tbf_len = 4 + core::mem::size_of::<tc::TcPolice>();
        let mut tbf_payload: Option<&[u8]> = None;
        let mut i = 0;
        while i + 4 <= bytes.len() {
            let nla_len = u16::from_ne_bytes(bytes[i..i + 2].try_into().unwrap()) as usize;
            let nla_type = u16::from_ne_bytes(bytes[i + 2..i + 4].try_into().unwrap());
            if nla_type == police::TCA_POLICE_TBF
                && nla_len == tbf_len
                && i + nla_len <= bytes.len()
            {
                tbf_payload = Some(&bytes[i + 4..i + nla_len]);
                break;
            }
            i += 4;
        }
        let payload = tbf_payload.expect("TCA_POLICE_TBF not found");

        // tc_police layout: index(u32), action(i32), limit(u32),
        // burst(u32), mtu(u32), then `rate: tc_ratespec` =
        // { cell_log:u8, linklayer:u8, overhead:u16, ... }.
        // So rate.cell_log is at offset 20, rate.linklayer at 21.
        let cell_log = payload[20];
        let linklayer = payload[21];
        assert!(cell_log != 0, "cell_log must be non-zero for police");
        assert_eq!(
            linklayer, 0,
            "linklayer must be TC_LINKLAYER_UNAWARE (=0) for police rtab"
        );
    }
}
