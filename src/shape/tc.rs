...
pub mod htb {
    ...
    /// HTB protocol version expected by the kernel in `tc_htb_glob.version`.
    ///
    /// Kernel `net/sched/sch_htb.c` defines `HTB_VER 0x30011` (major 3,
    /// minor 0x11) and rejects with `-EINVAL` unless
    /// `gopt->version == HTB_VER >> 16`, i.e. exactly `3`.
    pub const HTB_VER: u32 = 3;
}
