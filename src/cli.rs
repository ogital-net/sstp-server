    ...
    I: Iterator<Item = V>,
{
    ...
    let (io_threads, auth_threads) = {
        let ncpus = available_parallelism().map_or(1, NonZeroUsize::get);
        compute_threads(ncpus, io_threads, auth_threads)?
    };
...
fn compute_threads(
    ...
    // When the user pins `-T` explicitly, raise the io-worker
    // default to at least `auth` so the cap below never silently
    // neuters their choice. Without this, on a small-`ncpus`
    // host (or under cgroup CPU restriction inside a build farm)
    // `ncpus.saturating_sub(auth)` collapses to 1 and `-T 4`
    // ends up materialising as one auth thread.
    let io = io_override.unwrap_or_else(|| {
        let derived = ncpus.saturating_sub(auth).max(1);
        if auth_override.is_some() {
            derived.max(auth)
        } else {
            derived
        }
...
mod tests {
    ...
    // ---- compute_threads ----------------------------------------------
    //
    // These tests exercise the policy in isolation by passing
    // `ncpus` explicitly, so the host's actual CPU topology (and
    // any cgroup that pins `available_parallelism()` to 1) doesn't
    // bleed into the assertions.

    fn ct(ncpus: usize, io: Option<usize>, auth: Option<usize>) -> (usize, usize) {
        let (io_nz, auth_nz) = compute_threads(ncpus, io, auth).expect("compute_threads ok");
        (io_nz.get(), auth_nz.get())
    }

    #[test]
    fn compute_threads_default_on_high_cpu_box() {
        // 16 vCPU: auth_default = max(2, 16/4) = 4, io = 16-4 = 12.
        assert_eq!(ct(16, None, None), (12, 4));
    }

    #[test]
    fn compute_threads_default_on_small_box() {
        // 1 vCPU: auth_default = max(2, 0) = 2, io = max(1, 1-2) = 1,
        // and auth is capped to io. End result: 1 io / 1 auth.
        assert_eq!(ct(1, None, None), (1, 1));
        // 2 vCPU: auth_default = 2, io = max(1, 2-2) = 1, cap → 1.
        assert_eq!(ct(2, None, None), (1, 1));
        // 4 vCPU: auth_default = 2, io = 2, cap → 2.
        assert_eq!(ct(4, None, None), (2, 2));
    }

    #[test]
    fn compute_threads_explicit_auth_not_capped_by_default_io() {
        // Regression for `cli::tests::short_auth_threads` panicking
        // under cgroup CPU restriction: `-T 4` on a 1-vCPU view
        // must not silently collapse to 1 thread.
        assert_eq!(ct(1, None, Some(4)), (4, 4));
        assert_eq!(ct(2, None, Some(4)), (4, 4));
        // On a beefy box the io default still wins where larger.
        assert_eq!(ct(32, None, Some(4)), (28, 4));
    }

    #[test]
    fn compute_threads_explicit_io_caps_auth() {
        // User picks 2 io threads explicitly; auth must follow.
        assert_eq!(ct(16, Some(2), None), (2, 2));
        // ... including against an explicit `-T` that exceeds io.
        // The cap is intentional: more auth than io makes no sense.
        assert_eq!(ct(16, Some(2), Some(8)), (2, 2));
    }

    #[test]
    fn compute_threads_zero_ncpus_treated_as_one() {
        // `available_parallelism()` should never return 0, but
        // the helper must defend against it rather than divide
        // by something pathological or panic on `NonZeroUsize`.
        assert_eq!(ct(0, None, None), (1, 1));
    }

    #[test]
    fn compute_threads_rejects_explicit_zero() {
        let err = compute_threads(8, Some(0), None).unwrap_err();
        assert!(matches!(err, ParseError::ZeroThreads));
        let err = compute_threads(8, None, Some(0)).unwrap_err();
        assert!(matches!(err, ParseError::ZeroThreads));
    }
}
