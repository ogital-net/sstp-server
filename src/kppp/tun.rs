...
use crate::ppp::frame::ProtocolId;
...
pub struct TunSession {
    ifname: String,
    ifindex: u32,
    /// Negotiated MTU (the value passed to [`Self::bring_up`] and
    /// installed on the netdev via netlink). Stored so the
    /// session-level NP filter can drop oversized inbound IP
    /// frames; nothing on the data path consults it.
    mtu: u32,
...
impl TunSession {
    ...
    pub fn bring_up(local: Ipv4Addr, peer: Ipv4Addr, mtu: u32) -> Result<Self, TunError> {
        ...
        tracing::trace!(
            ...
            %local,
            %peer,
            "TunSession::bring_up: programming tun netdev"
        );
    ...
    /// Negotiated MTU (the value installed on the netdev via
    /// netlink). The session-level NP filter uses this to drop
    /// oversized inbound IP frames before they hit the tun fd.
    #[must_use]
    pub fn mtu(&self) -> u32 {
        self.mtu
    }

    /// Write a raw IP body (no PPP header) to the tun fd. The
    /// session driver decodes the PPP frame to dispatch on the
    /// protocol field and passes the bare IP body here.
    ...
    ///
    /// Reads straight into `buf[4..]` and post-fills the 4-byte PPP
    /// header in place — no scratch, no per-packet body copy.
    pub async fn read_frame(&self, buf: &mut [u8]) -> io::Result<usize> {
        // Need at least the 4-byte PPP header + 1 byte to read the
        // IP version nibble.
        if buf.len() < 5 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                format!("read_frame buf too small: need ≥ 5, have {}", buf.len()),
            ));
        }
        let cap = buf.len() - 4;
        let cap_check = u32::try_from(cap).unwrap_or(u32::MAX);
        debug_assert!(
            cap_check >= MAX_TUN_MTU,
            "read_frame buf shorter than configured TUN MTU",
        );
        let n = self
            .async_fd
            .async_io(Interest::READABLE, |fd| read_fd(fd.as_fd(), &mut buf[4..]))
            .await?;
        if n == 0 {
            return Ok(0);
        }
        // RFC 791 §3.1 (IPv4) / RFC 8200 §3 (IPv6) — IP version is
        // the top nibble of the first body byte.
        let proto = match buf[4] >> 4 {
            4 => ProtocolId::Ip,
            6 => ProtocolId::Ipv6,
            other => {
                return Err(io::Error::new(
        ...
        // Uncompressed PPP header: Address (0xFF), Control (0x03),
        // 2-byte Protocol. RFC 1661 §2.
        buf[0] = 0xFF;
        buf[1] = 0x03;
        buf[2..4].copy_from_slice(&proto.as_u16().to_be_bytes());
        Ok(4 + n)
    }
}
