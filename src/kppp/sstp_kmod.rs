...
pub const SSTP_ABI_VERSION_MINOR: u16 = 3;
...
const SSTP_IOC_SEND_CONTROL: libc::c_ulong =
    super::ioctl::ioc_write::<SstpSendControlRaw>(SSTP_IOC_MAGIC, 0x85);
const SSTP_IOC_REKEY_TX: libc::c_ulong =
    super::ioctl::ioc_write::<SstpRekeyRaw>(SSTP_IOC_MAGIC, 0x86);
const SSTP_IOC_REKEY_RX: libc::c_ulong =
    super::ioctl::ioc_write::<SstpRekeyRaw>(SSTP_IOC_MAGIC, 0x87);
...
/// Wire-compatible mirror of `struct sstp_send_control` (UAPI v0.3).
#[repr(C)]
#[derive(Debug, Clone, Copy)]
struct SstpSendControlRaw {
    buf_len: u32,
    reserved: u32,
    buf: u64,
}

/// Wire-compatible mirror of `struct sstp_rekey` (UAPI v0.3).
#[repr(C)]
#[derive(Debug, Clone, Copy)]
struct SstpRekeyRaw {
    cipher_type: u32,
...
pub struct SstpStats {
    ...
    pub evt_dropped: u64,
    /// v0.3: control frames demuxed and queued for userspace.
    pub ctrl_frames_rx: u64,
    /// v0.3: control frames emitted via [`KmodSession::send_control`].
    pub ctrl_frames_tx: u64,
    /// v0.3: hard `kernel_sendmsg` failures and short writes
    /// (excludes -EAGAIN, which is surfaced to the caller).
    pub tx_send_errors: u64,
}
...
impl KmodSession {
    ...
    pub fn attach(tcp_fd: BorrowedFd<'_>, ppp_unit: i32, mtu: u32) -> Result<Self, KmodError> {
        tracing::trace!(
            ...
            ppp_unit,
            mtu,
            "SSTP_IOC_ATTACH: handing kTLS-equipped TCP fd + MTU to kmod"
        );
    ...
    /// Emit one SSTP control frame (C=1) through the kmod's TX path.
    ///
    /// `body` is the SSTP control-message body — i.e. everything
    /// after the 4-byte outer SSTP header ([MS-SSTP] §2.2.1). The
    /// kernel prepends the header (version 1.0, C=1, length =
    /// `4 + body.len()`) and writes the resulting frame to the
    /// kTLS socket as a single TLS record.
    ///
    /// Always non-blocking. On socket-buffer backpressure the
    /// kernel returns `EAGAIN`, surfaced here as
    /// [`io::ErrorKind::WouldBlock`]; callers should poll the
    /// session fd ([`Self::as_fd`]) for `EPOLLOUT` and retry.
    /// `EPIPE` (mapped to [`io::ErrorKind::BrokenPipe`]) means the
    /// session is closing; `EMSGSIZE` means `body` exceeds
    ...
    pub fn send_control(&self, body: &[u8]) -> io::Result<()> {
        ...
        let req = SstpSendControlRaw {
            buf_len: len,
            reserved: 0,
            buf: body.as_ptr() as u64,
        };
        // SAFETY: `self.session_fd` is a valid open fd. The ioctl
        // is `_IOW(...)` of `sizeof(SstpSendControlRaw)`; the kernel
        // reads the request struct and dereferences `req.buf` for
        // `req.buf_len` bytes synchronously inside the ioctl, both
        // of which point at memory live for the duration of this
        // call.
        let rc = unsafe {
            libc::ioctl(
                self.session_fd.as_raw_fd(),
    ...
    /// Install a fresh kTLS TX crypto context (TLS 1.3 `KeyUpdate`).
    ///
    /// Reserved in ABI v0.3 — the kernel returns `ENOSYS`. The
    /// shape mirrors `setsockopt(SOL_TLS, TLS_TX, ...)`:
    /// `crypto_info` is a `tls12_crypto_info_*` blob whose layout
    /// the kernel already knows for `cipher_type`.
    pub fn rekey_tx(&self, cipher_type: u32, crypto_info: &[u8]) -> io::Result<()> {
        self.rekey(SSTP_IOC_REKEY_TX, cipher_type, crypto_info)
    }

    /// Install a fresh kTLS RX crypto context. See [`Self::rekey_tx`].
    pub fn rekey_rx(&self, cipher_type: u32, crypto_info: &[u8]) -> io::Result<()> {
        self.rekey(SSTP_IOC_REKEY_RX, cipher_type, crypto_info)
    }
    ...
    fn rekey(&self, ioc: libc::c_ulong, cipher_type: u32, crypto_info: &[u8]) -> io::Result<()> {
        ...
        let req = SstpRekeyRaw {
            cipher_type,
            info_len,
            crypto_info: crypto_info.as_ptr() as u64,
        };
        // SAFETY: same justification as `send_control` — the ioctl
...
mod tests {
    ...
    fn sstp_ioctl_numbers_match_kernel() {
        ...
        // _IOR('S', 0x82, struct sstp_stats) — 96 bytes (12 × u64) in v0.3
        assert_eq!(SSTP_IOC_GETSTATS, 0x8060_5382);
        ...
        // _IOW('S', 0x85, struct sstp_send_control) — 16 bytes
        assert_eq!(SSTP_IOC_SEND_CONTROL, 0x4010_5385);
        // _IOW('S', 0x86, struct sstp_rekey) — 16 bytes
        assert_eq!(SSTP_IOC_REKEY_TX, 0x4010_5386);
        // _IOW('S', 0x87, struct sstp_rekey) — 16 bytes
        assert_eq!(SSTP_IOC_REKEY_RX, 0x4010_5387);
    }
    ...
    #[test]
    fn send_control_struct_is_16_bytes() {
        assert_eq!(core::mem::size_of::<SstpSendControlRaw>(), 16);
    }

    #[test]
    fn rekey_struct_is_16_bytes() {
        assert_eq!(core::mem::size_of::<SstpRekeyRaw>(), 16);
    }

    #[test]
    fn stats_struct_is_v03_size() {
        // 12 × u64 in v0.3 (was 9 in v0.2).
        assert_eq!(core::mem::size_of::<SstpStats>(), 96);
    }
