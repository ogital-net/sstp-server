...
// `close`, `Configure` channel variant) are kept for the kernel
// data-path layer. They have no caller in the binary today (the
// kmod path drives the unit fd directly) but are retained for the
// userspace fallback and for tests.
#![allow(dead_code)]
...
impl Unit {
    ...
    pub fn new() -> Result<Self, UnitError> {
        ...
        let name = format!("ppp{index}");
        Ok(Self { fd, index, name })
    }
    ...
    /// Interface name (`pppN`). Cached at construction; cheap.
    #[must_use]
    ...
    /// pptp, …).
    ///
    /// See [docs/data-path.md](../../docs/data-path.md) for the implications.
