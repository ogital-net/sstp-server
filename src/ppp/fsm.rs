...
impl Fsm {
    ...
    #[must_use]
    #[allow(dead_code)] // FUTURE: state inspection for control-socket `show session <id>` once exposed.
...
mod tests {
    ...
    // -------------------------------------------------------------
    // Coverage backfill: down events, nak loops, terminate flows,
    // restart-timer drains, code-reject branches.
    // -------------------------------------------------------------

    #[test]
    fn down_from_opened_returns_to_starting_with_notify() {
        let mut fsm = opened();
        let o = fsm.step(Event::Down);
        assert_eq!(o.new_state, State::Starting);
        assert!(o.notify.down);
    }

    #[test]
    fn down_from_closed_returns_to_initial() {
        let mut fsm = Fsm::new();
        fsm.step(Event::Up);
        assert_eq!(fsm.state(), State::Closed);
        let o = fsm.step(Event::Down);
        assert_eq!(o.new_state, State::Initial);
    }

    #[test]
    fn open_from_initial_starts_with_notify() {
        let mut fsm = Fsm::new();
        let o = fsm.step(Event::Open);
        assert_eq!(o.new_state, State::Starting);
        assert!(o.notify.started);
    ...
    #[test]
    fn close_from_starting_finishes_immediately() {
        let mut fsm = Fsm::new();
        fsm.step(Event::Open);
        let o = fsm.step(Event::Close);
        assert_eq!(o.new_state, State::Initial);
    ...
    #[test]
    fn close_from_reqsent_emits_terminate_req() {
        let mut fsm = Fsm::new();
        fsm.step(Event::Open);
        fsm.step(Event::Up);
    ...
    #[test]
    fn rcv_configure_ack_in_reqsent_advances_to_ackrcvd() {
        let mut fsm = Fsm::new();
        fsm.step(Event::Open);
        fsm.step(Event::Up);
        let o = fsm.step(Event::RcvConfigAck);
    ...
    #[test]
    fn rcv_configure_nak_in_reqsent_re_emits_request() {
        let mut fsm = Fsm::new();
        fsm.step(Event::Open);
        fsm.step(Event::Up);
        let o = fsm.step(Event::RcvConfigNak);
        assert_eq!(o.new_state, State::ReqSent);
        assert_eq!(o.send, Some(Send::ConfigureRequest));
    }

    #[test]
    fn rcv_configure_nak_in_opened_renegotiates() {
        let mut fsm = opened();
        let o = fsm.step(Event::RcvConfigNak);
        assert_eq!(o.new_state, State::ReqSent);
        assert_eq!(o.send, Some(Send::ConfigureRequest));
    ...
    #[test]
    fn rcv_configure_ack_in_opened_renegotiates_with_down_notify() {
        let mut fsm = opened();
        let o = fsm.step(Event::RcvConfigAck);
        assert_eq!(o.new_state, State::ReqSent);
        assert_eq!(o.send, Some(Send::ConfigureRequest));
    ...
    #[test]
    fn closed_rejects_unsolicited_acks_with_code_reject() {
        let mut fsm = Fsm::new();
        fsm.step(Event::Up); // Initial → Closed
        let o = fsm.step(Event::RcvConfigAck);
        assert_eq!(o.new_state, State::Closed);
    ...
    #[test]
    fn rcv_terminate_ack_in_stopping_finishes() {
        let mut fsm = opened();
        let _ = fsm.step(Event::RcvTerminateReq); // Opened → Stopping
    ...
    #[test]
    fn closing_restart_timeout_retransmits_terminate_req() {
        let mut fsm = opened();
        // Opened → Closing via Close.
        let _ = fsm.step(Event::Close);
        assert_eq!(fsm.state(), State::Closing);
        // First emit consumed one count; we still have budget.
        let o = fsm.step(Event::RestartTimeout);
        assert_eq!(o.new_state, State::Closing);
        assert_eq!(o.send, Some(Send::TerminateReq));
        assert_eq!(o.restart_timer, Some(RestartTimer::Start));
    ...
    #[test]
    fn closing_restart_timeout_to_minus_drops_to_closed() {
        let mut fsm = Fsm::new();
        fsm.max_terminate = 1;
        fsm.step(Event::Open);
        fsm.step(Event::Up);
        let _ = fsm.step(Event::Close); // ReqSent → Closing, sends TR (counter→0).
        assert_eq!(fsm.state(), State::Closing);
        let o = fsm.step(Event::RestartTimeout);
        assert_eq!(o.new_state, State::Closed);
        assert!(o.notify.finished);
    ...
    #[test]
    fn code_reject_catastrophic_in_opened_starts_termination() {
        let mut fsm = opened();
        let o = fsm.step(Event::RcvCodeRejCatastrophic);
        assert_eq!(o.new_state, State::Stopping);
        assert_eq!(o.send, Some(Send::TerminateReq));
        assert_eq!(o.restart_timer, Some(RestartTimer::Start));
        assert!(o.notify.down);
    ...
    #[test]
    fn code_reject_catastrophic_in_reqsent_drops_to_stopped() {
        let mut fsm = Fsm::new();
        fsm.step(Event::Open);
        fsm.step(Event::Up);
        let o = fsm.step(Event::RcvCodeRejCatastrophic);
    ...
    #[test]
    fn code_reject_permitted_is_noop() {
        let mut fsm = opened();
        let o = fsm.step(Event::RcvCodeRejPermitted);
        assert_eq!(o.new_state, State::Opened);
        assert!(o.send.is_none());
    }

    #[test]
    fn open_in_starting_is_noop() {
        let mut fsm = Fsm::new();
        fsm.step(Event::Open);
        let o = fsm.step(Event::Open);
        assert_eq!(o.new_state, State::Starting);
    ...
    #[test]
    fn restart_timeout_in_initial_is_noop() {
        let mut fsm = Fsm::new();
        let o = fsm.step(Event::RestartTimeout);
        assert_eq!(o.new_state, State::Initial);
        assert!(o.send.is_none());
