...
impl SessionEnd {
    ...
    pub fn to_terminate_cause(self) -> AcctTerminateCause {
        match self {
            ...
            SessionEnd::SessionTimeout => AcctTerminateCause::SESSION_TIMEOUT,
...
pub struct AcctSession {
    ...
    pub nas_port: u32,
    /// `Class` (RFC 2865 §5.25) — opaque RADIUS-supplied octet
    /// string echoed verbatim in every accounting record so the
    /// authenticator can correlate Auth and Acct sessions. `None`
    /// if the Access-Accept did not include a `Class` attribute.
...
impl AcctClient {
    ...
    pub async fn send(
        ...
        let (identifier, reply_rx) = {
            let mut all = self.state.lock().expect("acct state poisoned");
    ...
    loop {
        match socket.recv_from(&mut buf).await {
            Ok((n, peer)) if n >= 20 => {
                ...
                let id = datagram[1];
                let waiter = {
                    let mut all = state.lock().expect("acct state poisoned");
...
fn build(
    ...
    if let Some(class) = &session.class {
        // RFC 2865 §5.25: Class is opaque; echo the bytes verbatim.
        buf.add(rfc::attrs::CLASS, class.as_slice())?;
    }
    if delay_secs != 0 {
