...
pub struct AccessRequestCtx<'a> {
    ...
    /// `Called-Station-Id` (RFC 2865 §5.30) — typically the
    /// IP:port the client connected to.
    pub called_station_id: Option<&'a str>,
    /// `Connect-Info` (RFC 2869 §5.11) — free-form text describing
    /// the link, e.g. `"SSTP/TLS 1.3/AES-256-GCM"`.
    pub connect_info: Option<&'a str>,
}
...
pub fn apply_common(buf: &mut PacketBuffer, ctx: &AccessRequestCtx<'_>) -> Result<(), CodecError> {
    ...
    if let Some(ip) = ctx.nas_ip_address {
        buf.add(rfc::attrs::NAS_IP_ADDRESS, ip)?;
    }
    if let Some(csid) = ctx.calling_station_id {
        buf.add(rfc::attrs::CALLING_STATION_ID, csid)?;
    }
    if let Some(called) = ctx.called_station_id {
        buf.add(rfc::attrs::CALLED_STATION_ID, called)?;
    }
    if let Some(nid) = ctx.nas_identifier {
        buf.add(rfc::attrs::NAS_IDENTIFIER, nid)?;
    }
    if let Some(ci) = ctx.connect_info {
        buf.add(rfc::attrs::CONNECT_INFO, ci)?;
    }
